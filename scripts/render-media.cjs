...
async function main() {
  ...
  const transcriptPath = path.join(root, 'assets/demo/girelay-run-demo-transcript.txt');
  const frameDir = path.join(root, 'target/demo-frames');
  ...
  for (let index = 0; index < chunks.length; index += 1) {
    ...
    </style><header>girelay <span class="step">deterministic lifecycle ${index + 1}/${chunks.length}</span></header><pre>${escaped}</pre>`);
